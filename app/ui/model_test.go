...
func TestClearPendingInputState_ClearsAllFields(t *testing.T) {
	...
	assert.Zero(t, m.vim.pendingCount, "vim.count must be cleared")
	assert.Empty(t, m.vim.pendingChord, "vim.leader must be cleared")
	assert.Empty(t, m.vim.hint, "vim.hint must be cleared")
}

func TestVimState_ZeroValue(t *testing.T) {
	m := testModel([]string{"a.go"}, nil)
	assert.Equal(t, 0, m.vim.pendingCount, "vim.count must default to 0")
	assert.Empty(t, m.vim.pendingChord, "vim.leader must default to empty")
...
func TestStartSearch_ClearsVimState(t *testing.T) {
	...
	assert.Zero(t, m.vim.pendingCount, "startSearch must clear vim.count")
	assert.Empty(t, m.vim.pendingChord, "startSearch must clear vim.leader")
...
func TestStartAnnotation_ClearsVimState(t *testing.T) {
	...
	assert.Zero(t, m.vim.pendingCount, "startAnnotation must clear vim.count")
	assert.Empty(t, m.vim.pendingChord, "startAnnotation must clear vim.leader")
...
func TestStartFileAnnotation_ClearsVimState(t *testing.T) {
	...
	assert.Zero(t, m.vim.pendingCount, "startFileAnnotation must clear vim.count")
	assert.Empty(t, m.vim.pendingChord, "startFileAnnotation must clear vim.leader")
...
func TestHandleOverlayOpen_ClearsVimState_Help(t *testing.T) {
	...
	assert.True(t, handled, "ActionHelp must be handled by handleOverlayOpen")
	assert.Zero(t, model.vim.pendingCount, "help overlay entry must clear vim.count")
	assert.Empty(t, model.vim.pendingChord, "help overlay entry must clear vim.leader")
...
func TestHandleOverlayOpen_ClearsVimState_ThemeSelect(t *testing.T) {
	...
	assert.True(t, handled, "ActionThemeSelect must be handled by handleOverlayOpen")
	assert.Zero(t, model.vim.pendingCount, "theme-select overlay entry must clear vim.count")
	assert.Empty(t, model.vim.pendingChord, "theme-select overlay entry must clear vim.leader")
